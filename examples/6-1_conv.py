...
class ConvNet(Module):
    def __init__(self):
        super().__init__()
        self.conv1 = Conv(1, 8, kernel_size=3, padding=1)
        self.conv2 = Conv(8, 16, kernel_size=3, padding=1)
        self.fc    = Linear(16 * 8 * 8, 10)

    def forward(self, x, return_activations=False):
        conv1 = Node.relu(self.conv1(x))
        conv2 = Node.relu(self.conv2(conv1))
        flat = Node.flatten(conv2)
        fc = self.fc(flat)
        if return_activations:
            return conv1, conv2, fc
        return Node.softmax(fc)
